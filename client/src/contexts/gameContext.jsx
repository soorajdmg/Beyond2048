...
export const GameProvider = ({ children }) => {
  ...
  const saveGameStatsToDatabase = (gameResult) => {
    if (isAuthenticated) {
      ...
      const gameData = {
        ...
        moves: gameMoves, // Use current game moves
        result: gameResult,
  ...
  const initGame = useCallback(() => {
    ...
    setMergedTiles({});
    setNewTiles({});
    setGameMoves(0); // Reset current game moves counter
  ...
  const undo = () => {
    if (history.length > 0 && !isAnimating) {
      ...
      // Decrement move counter when undoing
      if (gameMoves > 0) {
  ...
  const checkGameOver = (board) => {
    ...
    let hasWon = false;
    let gameWon = won; // Store the current win state
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
        if (board[i][j] === 2048) {
          ...
          // If game not already marked as won, update stats for a win
          if (!gameWon) {
            const newGame = {
            ...
            updatePlayerStats({
              ...
              totalMoves: playerStats.totalMoves + gameMoves // Add current game moves to total
            });

            console.log("Saving game stats to database as a win...");
            saveGameStatsToDatabase('win');

            // Mark the game as won but don't end it
            gameWon = true;
          }

          hasWon = true;
          // Don't return here - continue checking if the game is over
        }
      }
    }
    ...
    if (!gameOver) {
      ...
      const newGame = {
        date: new Date().toLocaleDateString(),
        result: hasWon ? 'win' : 'loss',  // Mark as win if 2048 was reached
        score: score,
        highestTile: getHighestTile(board)
      };
      ...
      // Update total moves and reset winning streak for losses
      updatePlayerStats({
  ...
  const moveLeft = () => {
    ...
    // Update stats if moved
    if (moved) {
      // Increment current game moves counter
      setGameMoves(prev => prev + 1);

      // Update highest tile stat if needed
      updatePlayerStats({
